...
class SR830(object):
    ...
    def ReadValues(self):
        ...
        self._filter = self.inv_filterdict[int(self.ctrl.ask("OFLT?")[:-1])]
        self._filterslope = self.inv_filterslopedict[int(self.ctrl.ask("OFSL?")[:-1])]
        self._sensitivity = self.inv_sensitivitydict[int(self.ctrl.ask("SENS?")[:-1])]
        self._reserve = self.inv_reservedict[int(self.ctrl.ask("RMOD?")[:-1])]
        self._frequency = self.ctrl.ask_for_values("FREQ?")[0]
        self._amplitude = self.ctrl.ask_for_values("SLVL?")[0]
        self._harmonic = self.ctrl.ask_for_values("HARM?")[0]
        self._dcVoltage = self.ctrl.ask_for_values("AUXV? 1")[0]
    ...
    def getPhase(self):
        return self.ctrl.ask_for_values("PHAS?")[0]
    ...
    def hasOverload(self):
        ...
        return bool(self.ctrl.ask_for_values("LIAE? 0")[0])
    
    def gain(self, auxchannel=1):
        # not used with current measurement
        pass
        return float(self.gains[round(self.ctrl.ask_for_values("OAUX? " + `auxchannel`)[0]*2)/2])
        
    def measureRTheta(self):
        return self.ctrl.ask_for_values('SNAP? 3,4')
        
    def measureXY(self):
        return self.ctrl.ask_for_values('SNAP? 1,2')
